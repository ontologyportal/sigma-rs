// crates/core/src/persist/commit.rs
//
...
// with the IDs they already hold in `SyntacticLayer`.
//
...
pub(crate) fn write_axioms(
    ...
    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Info, target: "sigmakee_rs_core::persist", message: format!("write_axioms: {} sentence(s), session={:?}", sids.len(), session) });

    let mut wtxn = env.write_txn()?;
    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core::persist", message: format!("write txn opened") });
    ...
    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core::persist", message: format!("write_axioms: interned {} symbols", store.symbol_data.len()) });
    ...
    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core::persist", message: format!("write_axioms: kb_version bumped to {}", new_kb_version) });
    ...
    // -- 5. Commit -------------------------------------------------------------
    wtxn.commit()?;
    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Info, target: "sigmakee_rs_core::persist", message: format!("write_axioms: committed {} sentence(s), kb_version={}, features={:?}", sids.len(), new_kb_version, manifest.features) });
    Ok(())
}
...
fn write_sentence(
    ...
    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core::persist", message: format!("write_sentence: sid={} written", sid) });
    Ok(())
}
...
/// Persist the taxonomy portion of the semantic + translation layers.
/// Idempotent; the blob is rewritten from scratch on every call.
pub(crate) fn persist_taxonomy_cache(
    ...
    let blob = CachedTaxonomy {
        kb_version:           version,
        tax_edges:            layer.semantic.tax_edges_snapshot(),
        numeric_sort_cache:   layer.cache.numeric_sort_cache_snapshot(),
        numeric_ancestor_set: layer.cache.numeric_ancestor_set_snapshot(),
        poly_variant_symbols: layer.cache.poly_variant_symbols_snapshot(),
        numeric_char_cache:   layer.cache.numeric_char_cache_snapshot(),
    };
    env.put_cache(&mut wtxn, CACHE_KEY_TAXONOMY, &blob)?;
    wtxn.commit()?;
    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Info, target: "sigmakee_rs_core::persist", message: format!("taxonomy cache persisted: {} edges, {} numeric classes, \
...
pub(crate) fn persist_sort_annotations_cache(
    ...
    wtxn.commit()?;
    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Info, target: "sigmakee_rs_core::persist", message: format!("sort_annotations cache persisted: {} arg-sort entries, kb_version={}", blob.sorts.symbol_arg_sorts.len(), version) });
    Ok(())
}
...
pub(crate) fn backfill_cnf_tables(
    env:   &LmdbEnv,
    layer: &mut TranslationLayer,
) -> Result<BackfillReport, KbError> {
    use crate::canonical;
    use std::collections::HashMap;

    let axiom_sids: Vec<SentenceId> = layer.semantic.syntactic.roots.clone();

    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Info, target: "sigmakee_rs_core::persist", message: format!("cnf backfill: starting for {} axioms", axiom_sids.len()) });
    ...
    for sid in axiom_sids {
        ...
        let clauses = match crate::cnf::sentence_to_clauses(layer, sid) {
            Ok(cs) => cs,
            Err(e) => {
                crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Warn, target: "sigmakee_rs_core::persist", message: format!("cnf backfill: sid={} clausify failed: {}; skipping", sid, e) });
                clausify_failures += 1;
                continue;
            }
        };
    ...
    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Info, target: "sigmakee_rs_core::persist", message: format!("cnf backfill: done in {:?} -- {} axioms, {} clauses interned, {} formula hashes ({} clausify failures)", t0.elapsed(), report.axioms_processed, report.clauses_interned, report.formula_hashes, clausify_failures) });
...
pub(crate) fn persist_axiom_cache(
    ...
    wtxn.commit()?;
    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Info, target: "sigmakee_rs_core::persist", message: format!("axiom cache persisted ({}): {} axioms bincoded, kb_version={}", if mode_tff { "TFF" } else { "FOF" }, sid_map.len(), version) });
    Ok(())
}
