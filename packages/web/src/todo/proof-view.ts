 ...
 *  Called once from `vue/main.ts` AFTER the single root app mounts (this
 *  module is a transitive dependency of the app's own component tree, so
 *  its top-level code runs before the settings dialog exists in the DOM). */
export function initGenericVarsToggle() {
