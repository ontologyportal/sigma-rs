...
// # SDK migration
//
// Hover used to consume `sigmakee_rs_core::ManPage` directly and render its
// `documentation` text verbatim — `&%Symbol` cross-refs would land
// in the rendered markdown as raw `&%Symbol` text.  Now we go
// through [`sigmakee_rs_sdk::manpage_view`], which gives back a
// [`sigmakee_rs_sdk::ManPageView`] whose doc/term-format/format blocks are
// pre-segmented into [`sigmakee_rs_sdk::DocSpan::Text`] and
// [`sigmakee_rs_sdk::DocSpan::Link { text, target }`].  We render the Text
