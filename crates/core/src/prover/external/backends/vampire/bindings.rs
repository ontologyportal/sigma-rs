// // crates/core/src/prover/vampire/bindings.rs
//
...
pub fn extract_bindings(proof: &Proof, qvm: &QueryVarMap) -> Vec<ProofBinding> {
    ...
    let target_vars: Vec<String> = qvm
        .free_vars
        .iter()
        .filter_map(|&i| Some(format!("X{}", qvm.var_mapping.get(&i)?.0)))
        .collect();
    crate::log!(Debug, "sigmakee_rs_core::bindings", format!("extract_bindings: {} steps, target_vars={:?}, idx_to_kif={:?}", steps.len(), target_vars, qvm.var_mapping));
    for (i, (f, rule, prems)) in steps.iter().enumerate() {
        crate::log!(Trace, "sigmakee_rs_core::bindings", format!("  step {}: rule={:?} prems={:?} conclusion={:?}", i, rule, prems, f));
    }
...
impl AliasTracker {
    ...
    fn finalise(&self, qvm: &QueryVarMap) -> Vec<ProofBinding> {
        ...
        crate::log!(Debug, "sigmakee_rs_core::bindings", format!("finalise: extracted {} binding(s)", out.len()));
        out
    }
}
...
fn qvm_kif_name(qvm: &QueryVarMap, vampire_var: &str) -> String {
    ...
    let f = qvm.var_mapping
        .iter().find(|(_, i)| i.0 == idx);
    if let Some(mapping) = f {
        mapping.1.1.clone()
    } else {
        vampire_var.to_string()
    }
}
...
mod tests {
    ...
    fn qvm_with(idx_to_kif: &[(SymbolId, (u32, String))], free: &[SymbolId]) -> QueryVarMap {
        QueryVarMap {
            var_mapping: idx_to_kif.iter().map(|(i, n)| (*i, n.clone())).collect(),
            free_vars: free.to_vec(),
        }
    }
    ...
    fn qvm_kif_name_lookup() {
        let qvm = qvm_with(&[(10, (0, String::from("?A"))), (23, (1, String::from("?B")))], &[0, 1]);
        assert_eq!(qvm_kif_name(&qvm, "X0"), "?A");
        assert_eq!(qvm_kif_name(&qvm, "X1"), "?B");
