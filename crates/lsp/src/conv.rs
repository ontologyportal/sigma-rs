...
//! `Range`, `Diagnostic`) and the KB's native types (`String` file tag,
//! byte offset, `Span`, `Diagnostic`).  Everything LSP-specific lives here;
//! every other module in the crate speaks sigmakee-rs-core types.
//!
//! LSP positions are UTF-16 code units by default.  The server advertises
//! UTF-8 support but defaults to UTF-16 for the widest compatibility.
...
/// Best-effort disk read of a `file://` URL into a `Rope`.
///
...
///
/// LSP positions are 0-based.  The UTF-16 column is the count of UTF-16 code
/// units from the line start up to the byte offset.
...
///
/// Inverse of [`offset_to_position`].  Returns a clamped offset when the
/// position is past the end of its line; returns `rope.len_bytes()` when the
...
/// Convert a sigmakee-rs-core [`KbDiagnostic`] into an LSP [`LspDiagnostic`]
/// relative to `rope`.
///
/// The LSP `code` is `"<kind>/<code>"` (e.g. `"parse/kif/unbalanced-parens"`)
/// so clients can filter by category with a prefix match.
...
/// Placeholder URL used only when a related-info span has no valid file tag.
