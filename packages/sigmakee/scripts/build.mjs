...
function lockedBindgenVersion() {
  ...
  if (!m)
    throw new Error("could not find wasm-bindgen in the workspace Cargo.lock");
  return m[1];
}
...
if (existsSync(localBindgen)) {
  ...
  capture("wasm-bindgen", ["--version"])?.trim().split(/\s+/)[1] === wanted
) {
...
if (
  !capture("rustup", ["target", "list", "--installed"])?.includes(WASM_TRIPLE)
) {
...
run("cargo", [
  "build",
  "--manifest-path",
  join(WORKSPACE_ROOT, "crates", "wasm", "Cargo.toml"),
  "--target",
  WASM_TRIPLE,
  "--release",
]);
...
const wasmOpt = existsSync(localWasmOpt)
  ? localWasmOpt
  : has("wasm-opt")
    ? "wasm-opt"
    : null;
if (wasmOpt) {
  ...
  console.log(
    `==> Optimizing .wasm with wasm-opt -Oz (${wasmOpt === localWasmOpt ? "binaryen devDependency" : "PATH"})`,
  );
  const r = spawnSync(
    wasmOpt,
    [
  ...
  if (r.status === 0) {
    ...
    console.log(
      `    optimized: ${before} -> ${after} bytes (-${(100 * (1 - after / before)).toFixed(1)}%)`,
    );
  } else {
    rmSync(`${wasmFile}.opt`, { force: true });
    console.warn(
      `    WARNING: wasm-opt failed (exit ${r.status}); keeping the unoptimized .wasm`,
    );
  }
} else {
  console.warn(
    "==> WARNING: no wasm-opt found (run `npm install`); shipping an unoptimized .wasm",
  );
}
