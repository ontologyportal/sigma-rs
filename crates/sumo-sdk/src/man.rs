//! Man-page view: a structured projection of [`sumo_kb::ManPage`]
...
//!
//! ```no_run
//! use sumo_sdk::{manpage_view, DocSpan};
//!
//! // Caller owns the KB; the SDK never opens one.
//! let kb = sumo_kb::KnowledgeBase::new();
...
use sumo_kb::{KnowledgeBase, ManKind, ManPage, ParentEdge, SentenceId, SortSig};
// Note: `SentenceRef` (the (position, sid) pair stored in
// `ManPage::ref_args`) is `pub` but its module is `pub(crate)` in
// sumo-kb, so we can't name it here.  We never need to — access goes
