// crates/sumo-kb/src/cnf2.rs
// #[cfg(all(feature = "cnf", feature = "integrated-prover"))]
//
// Vampire-backed CNF clausification.
//
// This is the replacement for the hand-rolled clausifier in `cnf.rs` (which
// is scheduled for deletion in Phase 5 of the clause-dedup work).  It drives
// the KIF -> `ir::Problem` -> Vampire NewCNF -> `ir::Clause` pipeline added
// in the preceding phases and translates the resulting `ir::Clause`s into
// the crate-local `Clause` / `CnfLiteral` / `CnfTerm` shape that feeds
