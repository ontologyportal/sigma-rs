/**
 * Contribute tab state: the list of unpushed changes and the pull request
 * that carries them upstream to `ontologyportal/sumo`.
 *
 * The panel acts on a SELECTION, not on whichever file the editor happens to
 * show: every tracked change (see changes.ts) is listed with a checkbox, and
 ...
 *
 * The GitHub token comes from the signed-in OAuth session (auth.ts) -- there
 * is no manual token entry here any more; a signed-out user is sent to the
 * login dialog the moment they try to open this panel.
 *
 * Reactive state (not DOM ids) so `EditTab.vue` (which hosts this panel,
 * since it lives inside the Edit tab's markup) can bind to it directly. The
 * change list itself stays an HTML-string render (`renderChangeList`,
 * unchanged from before this went reactive) sunk into a ref and read via
 * delegated click/change/input handlers -- the row shape (checkboxes, path
 * inputs, per-row diff links, GitHub state chips) is intricate enough that a
 * from-scratch `v-for` re-templating would be a much bigger, harder-to-verify
 * change than swapping its DOM sink for a ref.
 *
 * Most of the exported surface here is read/written by `refreshChangeUi`,
 * `openDiffDialog`, or `checkStaleOnOpen` (the external entry points
 * `kb.ts`/`boot.ts`/`edit.ts` call), or by the private `selected`/`localPaths`
 * selection state those share with the change-list click/change handlers --
 * that coupling is why it all stays here rather than in the component. The
 * one piece that's genuinely standalone (the change list's collapse/expand
 * toggle) lives in `EditTab.vue` directly.
 */

import { ref } from 'vue';
import { SUMO } from '../constants.ts';
import { state } from '../state.ts';
import { $, esc, escAttr, isDarkTheme, targetEl } from '../dom.ts';
import { contributeFiles } from '../github.ts';
import { currentGithubToken } from '../github-api.ts';
import { openLoginDialog } from '../auth.ts';
import {
  changeRows, isActionable, markProposed, refreshUpstreamShas, refreshProposals,
  fetchUpstreamText,
} from '../changes.ts';
import type { ChangeRow } from '../changes.ts';
import { updateConstituentText } from '../kb.ts';
import { isBufferDirty } from './edit.ts';
import { loadMonaco } from '../editor/monaco.ts';

export const ghStatusText = ref('');
export const ghStatusIsError = ref(false);
function ghSetStatus(text, bad = false) {
  ghStatusText.value = text;
  ghStatusIsError.value = bad;
}
...
export const ghBadgeCount = ref('');
export const ghBadgeHidden = ref(true);
export const ghBadgeStale = ref(false);
export const ghProposeTitle = ref('Submit changes to GitHub as a pull request');
export const ghChangesCountText = ref('(none)');
export const ghChangesListHtml = ref('');
export const ghSubmitLabel = ref('Create pull request');
export const ghSubmitDisabled = ref(true);
...
export function refreshChangeUi() {
  ...
  ghBadgeCount.value = n ? String(n) : '';
  ghBadgeHidden.value = !n;
  ghBadgeStale.value = rows.some((r) => r.stale);
  ghProposeTitle.value = n
    ? `GitHub — ${n} file${n === 1 ? '' : 's'} changed and not yet pushed`
    : 'Submit changes to GitHub as a pull request';

  ghChangesCountText.value = rows.length ? `(${rows.length})` : '(none)';
  ghChangesListHtml.value = renderChangeList(rows);
  updateSubmitLabel(rows);
}

function renderChangeList(rows) {
  if (!rows.length) {
    return '<div class="hint">No changes yet — files you edit and save appear here until they are pushed.</div>';
  }
  const groups = [
...
function updateSubmitLabel(rows) {
  ...
  ghSubmitLabel.value = prs.size === 1 ? `Update pull request #${[...prs][0]}` : 'Create pull request';
  ghSubmitDisabled.value = !chosen.length;
}
...
export function onGhChangesListChange(e) {
  const box = targetEl(e).closest<HTMLInputElement>('input[type=checkbox]');
  if (!box) return;
...
export function onGhChangesListInput(e) {
  const path = targetEl(e).closest<HTMLInputElement>('input.gh-path');
  if (path) localPaths.set(path.dataset.key, path.value);
}

export function onGhChangesListClick(e) {
  const act = targetEl(e).closest<HTMLElement>('a.gh-act');
  if (!act) return;
...
export const ghDiffTitle = ref('Local changes');
export const ghDiffMsg = ref('');
export const ghDiffTakeLabel = ref('Revert to upstream');
export const ghDiffStatus = ref('');
export const ghDiffTakeBusy = ref(false);
...
export async function openDiffDialog(row) {
  diffRow = row;
  ghDiffTitle.value = row.stale ? 'Upstream has changed' : 'Local changes';
  ghDiffMsg.value = row.stale
  ...
  ghDiffTakeLabel.value = row.stale ? 'Take upstream (discards your changes)' : 'Revert to upstream';
  ghDiffStatus.value = 'Loading upstream copy…';
  ($('ghDiffDialog') as HTMLDialogElement).showModal();
  try {
    ...
    ghDiffStatus.value = 'Could not load the upstream copy: ' + (e && e.message || e);
  }
}
...
export function closeDiffDialog() {
  diffRow = null;
  disposeDiff();
  ($('ghDiffDialog') as HTMLDialogElement).close();
}

export function onGhDiffDialogClose() {
  diffRow = null;
  disposeDiff();
}

export async function onGhDiffTakeClick() {
  const row = diffRow;
  if (!row) return;
  ghDiffTakeBusy.value = true;
  ghDiffStatus.value = 'Replacing your copy…';
  try {
    ...
    ghDiffStatus.value = String(e && e.message || e);
  } finally {
    ghDiffTakeBusy.value = false;
  }
}
...
export const ghPanelOpen = ref(false);

export async function onGhProposeClick() {
  if (!currentGithubToken()) { openLoginDialog(); return; }
  ghPanelOpen.value = !ghPanelOpen.value;
  if (!ghPanelOpen.value) return;
  ghSetStatus('');
  refreshChangeUi();
  const rows = changeRows();
...
export const ghSubmitBusy = ref(false);
export const ghResultHtml = ref('');

export async function onGhSubmitClick() {
  ...
  const openPrs = (rows: ChangeRow[]) => new Map(rows.filter((r) => r.proposed).map((r) => [r.proposed!.number, r.proposed]));
  ...
  // Defensive: ghPropose already gates opening this panel on being signed
  // in, but a session can expire while the panel is open.
  if (!token) { openLoginDialog(); return; }

  ghSubmitBusy.value = true;
  try {
    ...
    if (!chosen.length) {
      ghSetStatus('');
      ghResultHtml.value = landed.length
    ...
    ghSetStatus('');
    ghResultHtml.value = `<span style="color:var(--bad)">${esc(String(e && e.message || e))}</span>`;
  } finally {
    ghSubmitBusy.value = false;
