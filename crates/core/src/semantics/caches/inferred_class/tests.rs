...
fn classify_formula_infers_variable_classes() {
    ...
    assert_eq!(
        classes.len(),
        2,
        "both variables classified, got {classes:?}"
    );
    ...
    let root = l
        ...
        .find(|&sid| {
            l.syntactic
                ...
                .map(|o| matches!(o, crate::OpKind::Equal))
                .unwrap_or(false)
        })
        .expect("equality root");

    let jsc = l
        .classify_formula_scoped(root, Scope::Base)
        .get(&joseph)
        .cloned()
        .expect("Joseph classified");
    assert!(
        matches!(&jsc.class, ClassInference::Single(id) if *id == man),
        "Joseph = (FatherOfFn …) takes range Man, got {:?}",
        jsc.class
    );
}
...
fn classify_formula_symbol_equality_propagates_class() {
    ...
    let fp_sc = l
        ...
        .get(&fp)
        .cloned()
        .expect("FirstPresident classified");
    assert!(
        matches!(&fp_sc.class, ClassInference::Single(id) if *id == human),
        "FirstPresident inherits GeorgeWashington's class, got {:?}",
        fp_sc.class
    );
    // …and it equals GeorgeWashington's own inferred class.
    assert!(
        matches!(l.infer_class_scoped(gw, Scope::Base), ClassInference::Single(id) if id == human)
    );
}
...
fn classify_formula_skips_negated_atoms() {
    ...
    assert_eq!(
        classes.len(),
        1,
        "only ?X is classified (Bar is negated), got {classes:?}"
    );
    let sc = classes.values().next().unwrap();
    assert!(
        matches!(&sc.class, ClassInference::Single(id) if *id == foo),
        "?X should be Foo only, got {:?}",
        sc.class
    );
}
...
fn infer_class_sees_through_double_negation() {
    ...
    assert!(
        matches!(l.infer_class_scoped(x, Scope::Base), ClassInference::Single(id) if id == foo),
        "double-negated instance → ?X : Foo, got {:?}",
        l.infer_class_scoped(x, Scope::Base)
    );
}
...
fn infer_class_drops_singly_negated_instance() {
    ...
    let (x, _) = l.syntactic.sentence_vars(root).into_iter().next().unwrap();
    assert!(
        matches!(l.infer_class_scoped(x, Scope::Base), ClassInference::Single(id) if id == bar),
        "?X should be Bar only (Foo is negated), got {:?}",
        l.infer_class_scoped(x, Scope::Base)
    );
}
    ...
    for root in l.syntactic.root_sids().into_iter() {
        for (x, _) in l.syntactic.sentence_vars(root) {
            assert!(
                ...
                "?X under (not (and …)) must be Unknown, got {:?}",
                l.infer_class_scoped(x, Scope::Base)
            );
            checked += 1;
        }
...
fn infer_class_equality_is_order_independent() {
    ...
    assert!(
        matches!(l1.infer_class_scoped(b1, Scope::Base), ClassInference::Single(id) if id == human1),
        "B-first must be Human, got {:?}",
        l1.infer_class_scoped(b1, Scope::Base)
    );
    ...
    assert!(
        matches!(l2.infer_class_scoped(b2, Scope::Base), ClassInference::Single(id) if id == human2),
        "B-after-A must ALSO be Human (order-independent), got {:?}",
        l2.infer_class_scoped(b2, Scope::Base)
    );
}
...
fn infer_class_equality_chains_transitively() {
    ...
    for name in ["A", "B", "C"] {
        ...
        assert!(
            matches!(l.infer_class_scoped(id, Scope::Base), ClassInference::Single(c) if c == human),
            "{name} should be Human via the equality chain, got {:?}",
            l.infer_class_scoped(id, Scope::Base)
        );
    }
}
...
fn infer_class_domain_reads_all_positions() {
    ...
    match l.infer_class_scoped(foo, Scope::Base) {
        ClassInference::Multiple(v) => assert!(
            v.contains(&region) && v.contains(&district),
...
fn infer_class_from_taxonomy() {
    ...
    let dog = l.syntactic.sym_id("Dog").unwrap();
    assert!(
        matches!(l.infer_class_scoped(fido, Scope::Base), ClassInference::Single(id) if id == dog)
    );
}
...
fn infer_class_class_symbol_returns_class() {
    ...
    let dog = l.syntactic.sym_id("Dog").unwrap();
    assert!(matches!(
        l.infer_class_scoped(dog, Scope::Base),
        ClassInference::Class
    ));
}
...
fn infer_class_variable_in_antecedent() {
    ...
    let dog = l.syntactic.sym_id("Dog").unwrap();
    assert!(
        matches!(l.infer_class_scoped(*x_id, Scope::Base), ClassInference::Single(id) if id == dog)
    );
}
...
fn infer_class_most_specific_two_instance_atoms() {
    ...
    assert!(
        matches!(l.infer_class_scoped(*x_id, Scope::Base), ClassInference::Single(id) if id == special_dog)
    );
}
...
fn infer_class_from_domain() {
    ...
    assert!(
        matches!(l.infer_class_scoped(*x_id, Scope::Base), ClassInference::Single(id) if id == real_number)
    );
}
...
fn infer_class_equality_from_range() {
    ...
    let foo = l.syntactic.sym_id("Foo").unwrap();
    assert!(
        matches!(l.infer_class_scoped(*x_id, Scope::Base), ClassInference::Single(id) if id == foo)
    );
}
...
fn infer_class_relations() {
    ...
    assert!(
        matches!(l.infer_class_scoped(color, Scope::Base), ClassInference::Single(id) if id == relation)
    );
    assert!(
        matches!(l.infer_class_scoped(sheen, Scope::Base), ClassInference::Single(id) if id == relation)
    );
}
...
fn infer_class_unknown() {
    ...
    assert!(matches!(
        l.infer_class_scoped(color, Scope::Base),
        ClassInference::Unknown
    ));
    assert!(matches!(
        l.infer_class_scoped(red, Scope::Base),
        ClassInference::Unknown
    ));
    assert!(matches!(
        l.infer_class_scoped(color_fn, Scope::Base),
        ClassInference::Unknown
    ));
    ...
    let (x_id, _) = vars.iter().next().unwrap();
    assert!(matches!(
        l.infer_class_scoped(*x_id, Scope::Base),
        ClassInference::Unknown
    ));
}
...
fn infer_class_multi_result_not_domain() {
    ...
    let ClassInference::Multiple(types) = bob_types else {
        panic!("Bob should have multiple class inferences")
    };
