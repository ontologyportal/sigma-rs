...
//
// Incremental callback calling and registration
...
//
// Sub-components (SyntacticLayer, LmdbEnv, SineIndex, …) live below the
...
impl SinkGuard {
    /// Installs the sink for the current thread. KB functions should use 
    /// with_
...
// Most variants are constructed only by code outside sigmakee-rs-core (the SDK
// and CLI emit them through this same sink) or read only by progress-
// sink consumers, so the dead-code lint fires on the in-crate slice
// alone.  The full event taxonomy is part of the public API contract
// regardless.
#[allow(dead_code)]
#[derive(Debug, Clone)]
#[non_exhaustive]
pub enum ProgressEvent {
    ...
    // -- SDK-level (emitted by sigmakee-rs-sdk through this same sink) -------------
    ...
    /// SDK started an ask op (mirrors `AskInvoked` from sigmakee-rs-core but
...
/// Mirrors `log::Level` but is owned by sigmakee-rs-core so consumers don't
