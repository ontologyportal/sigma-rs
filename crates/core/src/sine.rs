...
// Why eager
// ---------
// The original sigmakee-rs-core SInE was a lazy cache that rebuilt the whole index
...
impl SineIndex {
    ...
    pub(crate) fn add_axiom(&mut self, store: &SyntacticLayer, sid: SentenceId) {
        self.stats.calls += 1;
    ...
    pub(crate) fn add_axioms<I>(&mut self, store: &SyntacticLayer, sids: I)
    where
        I: IntoIterator<Item = SentenceId>,
    {
    ...
    fn rebuild_from(&mut self, store: &SyntacticLayer, sids: &[SentenceId]) {
        ...
        crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core::sine", message: format!("SineIndex::rebuild_from: {} axioms, {} symbols, {} trigger entries", self.axiom_count(), self.sym_axioms.len(), self.trigger_idx.values().map(|s| s.len()).sum::<usize>()) });
    }
    ...
    pub(crate) fn set_tolerance(&mut self, new: f32) {
        ...
        crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core::sine", message: format!("SineIndex::set_tolerance: rebuilt trigger relation at t={} \
    ...
    pub fn select(
        ...
        crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core::sine", message: format!("SineIndex::select: {} seed syms -> {} axioms ({} syms visited, depth {})", seed_syms.len(), selected.len(), visited_syms.len(), depth) });
        selected
    }
}
...
///
/// Crate-internal because `SyntacticLayer` is not part of the public API.
...
mod tests {
    ...
    /// SyntacticLayer's axiom-symbol helper so generality counts reflect reality.
    ...
    fn build_eager(store: &SyntacticLayer, axioms: &[SentenceId], tolerance: f32) -> SineIndex {
    ...
    fn fast_path_removes_symbol_when_bump_crosses_threshold() {
        ...
        let mut store = SyntacticLayer::default();
        let errs = crate::syntactic::load_kif(&mut store, "(subclass Human Animal)", "a");
        ...
        let errs = crate::syntactic::load_kif(&mut store, "(subclass Fruit Food)", "b");
        assert!(errs.is_empty());
        let a1 = *store.roots.last().unwrap();
        store.register_axiom_symbols(a1);
        idx.add_axiom(&store, a1);
