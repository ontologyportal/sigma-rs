...
//! let mut kb = sumo_kb::KnowledgeBase::open("./sumo.kb".as_ref()).unwrap();
//!
...
impl<'a> LoadOp<'a> {
    ...
    pub fn run(self) -> SdkResult<LoadReport> {
        ...
        // whether to keep it).
        let mut pending: Vec<(String, Vec<sumo_kb::SentenceId>, Vec<sumo_kb::SentenceId>)> =
        ...
        if let Some(p) = progress.as_deref_mut() {
            p.emit(&ProgressEvent::PromoteStarted {
                session: sumo_kb::session_tags::SESSION_LOAD.to_string(),
            });
        }
        ...
        for (tag, removed_sids, added_sids) in &pending {
            kb.persist_reconcile_diff(removed_sids, added_sids).map_err(|e| {
                log::error!(target: "sumo_sdk::load",
