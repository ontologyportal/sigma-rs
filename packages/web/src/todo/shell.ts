 ...
 *
 * `initShellChrome` wires all of it up -- called once from `vue/main.ts`
 * AFTER the single root app mounts. This module (like router.ts, auth.ts,
 * kb.ts, kb-cache.ts, proof-view.ts) is a transitive dependency of the app's
 * own component tree, so its top-level code runs while that tree is still
 * being imported/resolved, before any of its markup (header, settings
 * dialog, version dialog) actually exists in the DOM -- hence a function to
 * call post-mount rather than bare top-level `$(id).addEventListener`. */
...
export function initShellChrome() {
  ...
  // -- Bug report: no anonymous issue creation via the GitHub API, so this
  // just opens a prefilled "new issue" page against this app's own repo. ----
