 ...
 * HEAD moves (see kb-cache.js), which would otherwise take every unpushed
 * edit with it.
 */

import { EDITS_KEY, SUMO, rawUrl } from './constants.js';
import { state } from './state.js';
import { githubApi, fetchSumoTree } from './github-api.js';
import { fromBase64 } from './github.js';
...
export function markProposed(entries, pr) {
  for (const e of entries) {
    ...
    const rec = index[k] || {
      ...
      baseBlobSha: null, savedAt: Date.now(),
    };
    rec.path = e.path;
