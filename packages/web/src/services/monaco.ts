/**
 * Monaco: the lazy npm-package load, the `kif` and `tptp` languages, the
 * diagnostic marker conversion, and the registry of the one editor whose
 * model carries an LSP document identity. Shared by the Edit tab's IDE, the
 * Ask/Tell panes, the read-only TPTP preview, and the contribution diff.
 */
...
let monacoLoadPromise: Promise<MonacoNs> | null = null;
let monaco: MonacoNs | null = null;
let currentLspEditor: Monaco.editor.IStandaloneCodeEditor | null = null;

/** Load Monaco once and register the languages and themes. A rejected load
 *  is not memoized, so a later call retries.
 *
...
/** The Monaco namespace once `loadMonaco` has resolved, else null. */
export function monacoNs(): MonacoNs | null {
  return monaco;
}

/** The theme id matching the page's light/dark state. */
export function themeName(dark: boolean): "kif-dark" | "kif-light" {
  return dark ? "kif-dark" : "kif-light";
}

/** Register (or clear, with null) the editor whose model carries an LSP
 *  document identity (the Edit tab's live buffer). The LSP-backed providers
 *  serve only this editor's model. */
export function setLspEditor(
  editor: Monaco.editor.IStandaloneCodeEditor | null,
): void {
  currentLspEditor = editor;
}

/** The editor registered via `setLspEditor`, if any. */
export function lspEditor(): Monaco.editor.IStandaloneCodeEditor | null {
  return currentLspEditor;
}

/** Monarch tokenizer mirroring `highlightKif`. */
const KIF_MONARCH: Monaco.languages.IMonarchLanguage = {
  defaultToken: "",
  tokenizer: {
    root: [
      { include: "@whitespace" },
    ...
    afterOpen: [
      { include: "@whitespace" },
      [
        /\b(?:and|or|not|forall|exists|equal)\b/,
...
/**
 * The LSP's context-aware completion: it understands head vs argument
 * position and filters argument suggestions to the head's declared domain
 * class, and its items carry documentation popups. Only the Edit tab's live
 * document has an LSP identity; other kif models get nothing from this
 * provider.
 *
 ...
 */
function lspCompletionProvider(
  m: MonacoNs,
  ...
  return {
    async provideCompletionItems(model, position, _context, token) {
      ...
      if (!tag || lspEditor()?.getModel() !== model) return { suggestions: [] };
      ...
      try {
        ...
        return {
          suggestions,
          incomplete: serverIncomplete || items.length > CAP,
        };
      } catch (e) {
        ...
        return {
          suggestions: placeholderSuggestion(
            m,
            range,
            "Completion unavailable (see console)",
            word.word,
          ),
        };
      }
 ...
 * tokenizer above can't make since it never consults the KB. Only the Edit
 * tab's live document has an LSP identity; other kif models get no semantic
 * tokens (Monarch's lexical highlighting still applies to them, and to
 * comments/strings/delimiters here too -- semantic tokens only cover the
 * LSP's classified subset and layer on top).
 */
function lspSemanticTokensProvider(
  _m: MonacoNs,
...
function defineKifLanguage(m: MonacoNs): void {
  ...
  m.languages.setLanguageConfiguration("kif", {
    ...
    // getWordAtPosition return whole SUMO terms, hyphens included -- the
    // Monaco default stops at "-" and would hand back a fragment.
  ...
  m.languages.registerDocumentFormattingEditProvider("kif", {
    async provideDocumentFormattingEdits(model) {
      const local = (): Monaco.languages.TextEdit[] => [
        { range: model.getFullModelRange(), text: formatKif(model.getValue()) },
      ];
      const tag = lspOpenTag();
...
const TPTP_MONARCH: Monaco.languages.IMonarchLanguage = {
  defaultToken: "",
  tokenizer: {
    root: [
      [/%.*$/, "comment"],
      ...
      [
        /\b(?:axiom|hypothesis|conjecture|negated_conjecture|type|plain)\b/,
        "keyword",
      ],
      [/[!?]\[/, "keyword"],
...
function defineTptpLanguage(m: MonacoNs): void {
...
const SEVERITY_TO_MONACO: Record<
  string,
...
export function diagsToMarkers(
  ...
  return diags.map((d) => ({
    ...
    severity:
      m.MarkerSeverity[SEVERITY_TO_MONACO[d.severity]] || m.MarkerSeverity.Info,
  }));
}
