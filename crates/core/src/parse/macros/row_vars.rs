...
//
// The expansion runs inside `syntactic::load_kif`, immediately after initial
// parsing.  By the time a formula enters the SyntacticLayer or SemanticLayer it
...
///
/// Used in `syntactic::load_kif` as a fast pre-check: if a top-level parsed
...
pub fn expand_row_vars(node: &AstNode, parser: &Parser) -> Vec<AstNode> {
    ...
    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core", message: format!("Macro expansion: row variable expansion: expanding {}", str_rep) });
    ...
    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core", message: format!("Macro expansion: row variable expansion: expanded into {} new sentences", result.len()) });
    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core", message: format!("{}", result.join("\n")) });
