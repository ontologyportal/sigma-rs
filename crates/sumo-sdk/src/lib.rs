//! `sumo-sdk` — programmatic Rust API over [`sumo_kb`].
//!
...
//! codes, no `log::error!`-then-bail — `sumo-sdk` is meant to be
...
//! [`sumo_kb::KnowledgeBase::new`] (in-memory) or
//! [`sumo_kb::KnowledgeBase::open`] (LMDB) directly and hand the KB
...
//! [`ProgressSink`].  See the [`sumo_kb::progress`] module for the
//! event taxonomy.  Three usage shapes:
//!
//! ```no_run
//! use std::sync::Arc;
//! use sumo_sdk::{IngestOp, ProgressEvent, ProgressSink};
//!
...
//! module's docs][sumo_kb::progress] for the full event list.
//!
//! [progress-module]: sumo_kb::progress
//!
//! # Feature flags
//!
//! Each flag forwards 1:1 to the matching `sumo-kb` feature.
//! Activating a flag may *add* public API surface — code matching on
//! gated types must mirror the same gates.
//!
//! | Flag | Default | Adds |
//! |---|---|---|
//! | `persist` | ✓ | [`LoadOp`], LMDB-backed [`sumo_kb::KnowledgeBase::open`] |
...
//! `profile_call!` site inside `sumo-kb` emits
...
//!       path exists in `sumo-kb`.  Worst-case mitigation is to drop
...
//!   [`sumo_kb::KnowledgeBase::tell`] directly.
...
//! # See also
//!
//! - [`sumo_kb`] — the core knowledge-base library this SDK wraps.
...
// Progress types come from sumo-kb verbatim — the SDK re-exports
// rather than wrapping.  Consumers see one trait + one enum across
// both layers.  See `sumo_kb::progress` for the full taxonomy.
pub use sumo_kb::{DynSink, LogLevel, ProgressEvent, ProgressSink};
pub use report::{
...
// also importing `sumo_kb` directly.
#[cfg(feature = "ask")]
pub use sumo_kb::{Binding, KifProofStep, ProverStatus};
#[cfg(feature = "ask")]
pub use sumo_kb::TestCase;
#[cfg(feature = "ask")]
