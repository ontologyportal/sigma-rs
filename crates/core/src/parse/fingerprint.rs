// crates/core/src/parse/fingerprint.rs
//
...
/// in the `SyntacticLayer`.  Produces the **same hash bytes** as
...
pub(crate) fn sentence_fingerprint_from_store(sid: SentenceId, store: &SyntacticLayer) -> u64 {
    let mut h = Xxh64::new(SEED);
    hash_sentence_plain(&mut h, sid, store);
    h.digest()
}

fn hash_sentence_plain(h: &mut Xxh64, sid: SentenceId, store: &SyntacticLayer) {
...
fn hash_element_plain(h: &mut Xxh64, elem: &Element, store: &SyntacticLayer) {
    match elem {
...
// a `Sentence` in the SyntacticLayer (KB-side path).  They produce
...
/// in the `SyntacticLayer` directly, without a round-trip to AST.  Produces
/// identical hash bytes when the underlying formulas are
/// alpha-equivalent.  Applies the same outer-forall stripping.
#[cfg(feature = "ask")]
pub(crate) fn sentence_canonical_fingerprint(sid: SentenceId, store: &SyntacticLayer) -> u64 {
    let mut h = Xxh64::new(SEED);
    let mut vars: HashMap<String, u32> = HashMap::new();
    let mut rows: HashMap<String, u32> = HashMap::new();
...
/// sentence is reached.
#[cfg(feature = "ask")]
fn strip_leading_forall_sid(mut sid: SentenceId, store: &SyntacticLayer) -> SentenceId {
    loop {
...
mod tests {
    ...
    fn plain_matches_between_ast_and_store() {
        ...
        let sid = kb.syntactic().file_roots["t.kif"][0];
    ...
    fn canonical_matches_between_ast_and_store() {
        ...
        let sid = kb.syntactic().file_roots["test.kif"][0];
