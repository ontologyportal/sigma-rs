...
fn assign_descendents_to_sort(
    store: &EntryCache<SymbolId, Sort>,
    semantic: &SemanticLayer,
    sym_id: SymbolId,
    sort: Sort,
) {
    if store.get(&sym_id) == Some(sort) {
        return;
    ...
    semantic
        .children_of(sym_id)
        .iter()
        .for_each(|(child_id, rel_type)| {
            if !matches!(rel_type, TaxRelation::Subclass) {
                return;
            }
