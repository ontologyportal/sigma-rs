//! `textDocument/hover` handler.  Maps the cursor offset to a symbol via
//! `KnowledgeBase::symbol_at_offset`, then renders the symbol's man-page
//! view as CommonMark for the client.
//!
//! The markdown surfaces the most useful fields: kind, direct parents, one
//! documentation paragraph, and a signature summary.
...
/// Render a [`DocBlock`]'s spans as inline markdown.  Plain text passes
/// through verbatim; cross-ref links render as `**Symbol**`.
