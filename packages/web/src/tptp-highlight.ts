 ...
 *  `prover-config.ts`'s `proofLanguage`) is set to `"tptp"`. Mirrors
 *  `kif-highlight.ts`'s span-class vocabulary and `linkSymbols`/`focusSymbol`
 *  behavior so a proof step looks the same either way, just in a different
 *  formula dialect. */

import { esc, escAttr } from './dom.ts';
import { isInternalSymbol } from './kif-highlight.ts';

// Statement framing / annotation vocabulary — not SUMO symbols, never linked.
const TPTP_KEYWORDS = new Set([
  'fof', 'cnf', 'tff', 'thf', 'tpi',
  'axiom', 'hypothesis', 'conjecture', 'negated_conjecture', 'plain',
  'lemma', 'theorem', 'corollary', 'definition', 'assumption', 'unknown',
  'type', 'include', 'inference', 'introduced', 'file', 'status',
  'esa', 'thm', 'cth', 'sab',
]);
...
function unquoteAtom(src) {
  return src.slice(1, -1).replace(/\\(.)/g, '$1');
}

export function highlightTptp(src: string, { focusSymbol, linkSymbols }: { focusSymbol?: string; linkSymbols?: boolean } = {}) {
  ...
  while ((m = TPTP_TOKEN_RE.exec(src))) {
    ...
    else if (dstr) { out += `<span class="tok-str">${esc(dstr)}</span>`; afterOpenParen = false; }
    else if (qstr) {
    ...
    else if (paren) { out += `<span class="tok-paren">${esc(paren)}</span>`; afterOpenParen = paren === '(' || paren === '['; }
