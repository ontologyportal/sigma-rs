    ...
    fn prove(&self, tptp: &str, opts: &ProverOpts) -> ProverResult {
        ...
        if let Some(path) = &self.tptp_dump_path {
            if let Err(e) = write_file(path, tptp) {
                crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Warn, target: "sigmakee_rs_core::prover", message: format!("failed to write TPTP dump to {}: {}", path.display(), e) });
            } else {
                crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Info, target: "sigmakee_rs_core::prover", message: format!("wrote TPTP dump: {}", path.display()) });
            }
        }
        ...
        crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core::prover", message: format!("vampire: {} {} /dev/stdin", self.vampire_path.display(), args.join(" ")) });
        crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Info, target: "sigmakee_rs_core::prover", message: format!("starting vampire prover") });
        ...
        if let Some(mut stdin) = child.stdin.take() {
            if let Err(e) = stdin.write_all(tptp.as_bytes()) {
                crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Warn, target: "sigmakee_rs_core::prover", message: format!("failed to write to vampire stdin: {}", e) });
            }
        }
        ...
        match output {
            ...
            Ok(out) => {
                ...
                crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Info, target: "sigmakee_rs_core::prover", message: format!("vampire result: {:?}", status_label(&status)) });
