...
pub(crate) fn inject_domain_guards(
    ...
    for &sid in seed {
        ...
        for (var_id, classes) in var_uses {
            ...
            let Some(class_id) = most_specific(semantic, &dedup) else {
                continue;
            };
            ...
            if let Some(set) = existing.get(&var_id) {
                if set
                    .iter()
                    .any(|g| *g == class_id || semantic.has_ancestor(class_id, *g))
                {
                    continue;
                }
            }
    ...
    for plan in plans {
        let guard_sids: Vec<SentenceId> = plan
            .guards
            .into_iter()
            .map(|(var_id, class_id)| {
                let instance_sym = syntactic
                    .sym_name(instance_id)
                    .expect("`instance` symbol interned");
                let class_sym = syntactic
                    .sym_name(class_id)
        ...
        let mut and_elems: ElementVec =
            ElementVec::with_capacity(1 + guard_sids.len() + existing_conjuncts.len());
...
pub(super) fn most_specific(semantic: &SemanticLayer, candidates: &[SymbolId]) -> Option<SymbolId> {
    candidates.iter().copied().find(|&c| {
        candidates
            .iter()
            .all(|&other| semantic.has_ancestor(c, other))
    })
}
