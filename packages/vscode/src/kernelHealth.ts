...
export async function checkKernelHealth(
  ...
  if (configured.length > 0) {
    ...
    if (fs.existsSync(resolved)) {
      ...
      output.appendLine(
        `[health] kernel binary missing at configured path: ${resolved}`,
      );
      return {
        status: "missing",
        attempted: resolved,
...
function findOnPath(binary: string): string | undefined {
  ...
  const exts: string[] = isWin
    ? (process.env.PATHEXT ?? ".EXE;.BAT;.CMD;.COM")
        .split(";")
        .map((e) => e.toLowerCase())
    : [""];
  ...
  for (const dir of pathEnv.split(pathSep)) {
    ...
    for (const ext of exts) {
      ...
      const name =
        isWin && ext !== "" && binary.toLowerCase().endsWith(ext)
          ? binary
          : binary + ext;
  ...
  return new Promise((resolve) => {
    ...
    if (stdin !== undefined) {
      try {
        ...
        /* ignore; child.exit will clean up */
      }
    } else {
      child.stdin!.end();
    }
