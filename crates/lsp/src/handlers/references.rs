...
pub fn handle_references(state: &GlobalState, params: ReferenceParams) -> Option<Vec<Location>> {
    ...
    // get a clean list.
    // Occurrences no longer carry sentence ids or argument indices —
    // identity is the source span.  The declaration site is approximated
    // by span equality with the defining sentence's own occurrence span:
    // we resolve the defining sentence's file/range and skip occurrences
    // inside it when `include_decl` is off.  (Argument-position
    // precision was lost in the redesign; span containment is the
    // available signal.)
    let decl_span = kb
        .defining_sentence(kb.sym_name(sym_id).as_deref().unwrap_or(""))
        .map(|(_, span)| span);

    let mut locations: Vec<Location> = Vec::with_capacity(occurrences.len());
    for occ in occurrences {
        if !include_decl {
            if let Some(ref ds) = decl_span {
                if occ.span.file == ds.file
                    && occ.span.line >= ds.line
                    && occ.span.end_line <= ds.end_line
                {
                    continue;
                }
            }
