...
//! sumo_sdk::AskOp::new(&mut kb, "(holds ?X Animal)").run()?;
...
impl<'a> AskOp<'a> {
    ...
    pub fn run(self) -> SdkResult<AskReport> {
        ...
        for kif in &tells {
            log::debug!(target: "sumo_sdk::ask", "tell (session={}): {}", session, kif);
        ...
        // outcomes).
        if matches!(result.status, sumo_kb::ProverStatus::Unknown)
            && backend == ProverBackend::Subprocess
