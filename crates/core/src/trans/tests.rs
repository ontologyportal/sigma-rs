...
fn numeric_caches_populated_on_new() {
    let trans = make_trans("(subclass Integer RealNumber)(subclass RealNumber Quantity)");
    assert!(
        !trans.numeric_sorts.is_empty(),
        "numeric_sorts must be populated after TranslationLayer::new"
    );
    assert!(
        trans.numeric_ancestor_set.is_populated(),
        "numeric_ancestor_set must be populated after TranslationLayer::new"
    );
    assert!(
        trans.poly_variant_symbols.is_populated(),
...
fn on_change_taxonomy_invalidates_and_rebuilds_numeric_caches() {
    ...
    assert!(
        trans.numeric_ancestor_set.is_populated(),
        "ancestor set must be installed on construction"
    );
...
fn apply_change_taxonomy_addition_invalidates_and_reprimes() {
    ...
    trans.cascade(vec![Event::TaxonomyChanged {
        syms: vec![dog, animal],
    }]);
...
fn literal_equality_types_the_constant() {
    ...
    let cls = trans
        .semantic
        .infer_class_scoped(v, crate::semantics::types::Scope::Base);
    ...
    assert_eq!(
        sort.unwrap(),
        Sort::Real,
        "defining equality (equal V 40.0) must type V at $real"
    );
}
