//! Phase 1 smoke tests for `sigmakee-rs-sdk`'s `IngestOp`.
//!
...
use sigmakee_rs_sdk::{IngestOp, ProgressEvent, ProgressSink, SdkError};
...
fn parse_error_aborts_with_kb_error() {
    ...
    let result = IngestOp::new(&mut kb)
        ...
        .run();
    assert!(matches!(result, Err(sigmakee_rs_sdk::SdkError::Kb(_))));
}
...
fn add_dir_missing_returns_dir_read_error() {
    let mut kb = KnowledgeBase::new();
    let result = IngestOp::new(&mut kb)
        .add_dir("/tmp/sigmakee-rs-sdk-no-such-dir-xxxxx")
        .run();
