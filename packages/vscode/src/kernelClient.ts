...
export class SumoKernelClient {
  ...
  async ask(
    query: string,
    opts: { session?: string; timeoutSecs?: number } = {},
  ): Promise<AskResult> {
  ...
  async removeFile(filePath: string): Promise<RemoveFileResult> {
    return this.sendRequest<RemoveFileResult>("kb.removeFile", {
      path: filePath,
    });
  }
  ...
  async debug(
    ...
    if (opts.thoroughness !== undefined) {
      params.thoroughness = opts.thoroughness;
    }
    if (opts.scope !== undefined) {
      params.scope = opts.scope;
    }
    if (opts.timeoutSecs !== undefined) {
      params.timeoutSecs = opts.timeoutSecs;
    }
