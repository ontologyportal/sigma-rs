...
use sigmakee_rs_core::{KnowledgeBase, TellWarning};

fn kb() -> KnowledgeBase {
    KnowledgeBase::new()
}

fn dup_assertion_warnings(r: &sigmakee_rs_core::TellResult) -> Vec<&TellWarning> {
    r.warnings.iter().filter(|w| matches!(w, TellWarning::DuplicateAssertion { .. })).collect()
}

fn dup_axiom_warnings(r: &sigmakee_rs_core::TellResult) -> Vec<&TellWarning> {
