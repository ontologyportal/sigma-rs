...
/** Wires the header/dialog buttons -- called once from `vue/main.ts` AFTER
 *  the single root app mounts. This module is a transitive dependency of
 *  the app's own component tree (via tabs/contribute.ts), so its top-level
 *  code runs while that tree is still being imported/resolved, before the
 *  header or its dialogs exist in the DOM. */
export function initAuthDialogs() {
