 ...
 *
 * Reactive state (not DOM ids) so `BrowseTab.vue` can bind to it directly.
 * The popovers stay one-off DOM nodes appended to `document.body` (unchanged
 * from before this went reactive) -- `onTileDocClick`/`onTileRelationsClick`
 * are exported for the tiles' `@click` instead of being wired at module-load
 * time, since those tiles live inside a Vue-mounted region now and wouldn't
 * exist yet if this ran its old top-level `$(id).addEventListener`.
 */
...
export const stat = reactive({
  files: '—', symbols: '—', axioms: '—', rules: '—', classes: '—',
  instances: '—', relations: '—', doc: '—', docPct: 0, docTooltip: '',
  commit: '—', commitTooltip: '',
});
export const statNote = ref('');
...
export function onTileDocClick(tile: HTMLElement) {
  const s = lastStats;
  if (!s) return;
...
export function updateHomeNote(error?: unknown) {
  statNote.value = error ? `Could not read KB stats: ${error}`
...
export async function refreshHomeStats() {
  ...
  try {
    ...
    stat.files     = num(stats.files);
    stat.symbols   = num(stats.symbols);
    stat.axioms    = num(stats.axioms);
    stat.rules     = num(stats.rules);
    stat.classes   = num(stats.classes);
    stat.instances = num(stats.instances);
    stat.relations = num(stats.relations);
  ...
  try {
    ...
    stat.commit = d ? fmtDate(d) : 'unknown';
    stat.commitTooltip = d ? d.toString() : '';
  } catch (e) {
    stat.commit = '—';
    stat.commitTooltip = `Could not reach GitHub: ${e.message || e}`;
  }
}
