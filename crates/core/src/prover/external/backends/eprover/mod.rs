//! E command-line arguments and transcript parsing, shared by native and WASM runners.
...
use crate::prover::{
    result::{ProverMode, ProverResult, ProverStatus, ProverTimings, TerminationReason},
...
///   `--auto-schedule` portfolio). The bundled E build keeps SInE off here,
...
pub fn build_eprover_args(timeout_secs: u64) -> Vec<String> {
    let mut args = vec![
        "--auto".to_string(),
...
/// Parse E's captured output while retaining source axiom names and proof steps.
pub fn result_from_transcript(
    ...
    let status = determine_status(&combined, &mode);
    crate::log!(
        Info,
    ...
    let bindings = if matches!(mode, ProverMode::Prove) && combined.contains("SZS status Theorem") {
...
/// Combine exact-subset audits. Satisfiable subsets never establish whole-KB consistency.
pub fn aggregate_audits(results: &[ProverResult], available: usize) -> ProverResult {
    let mut aggregate = ProverResult::default();
    let mut supports = HashSet::new();
    for result in results {
        aggregate.timings.prover_run += result.timings.prover_run;
        if result.status == ProverStatus::Inconsistent {
            aggregate.status = ProverStatus::Inconsistent;
            let mut key: Vec<String> = result
                .proof_kif
                .iter()
                .filter(|step| step.premises.is_empty())
                .map(|step| {
                    step.source_sid
                        .map(|sid| sid.to_string())
                        .unwrap_or_else(|| step.formula.flat())
                })
                .collect();
            key.sort();
            key.dedup();
            if !result.proof_kif.is_empty() && supports.insert(key) {
                aggregate
                    .contradiction_proofs
                    .push(result.proof_kif.clone());
            }
        } else if aggregate.status != ProverStatus::Inconsistent
            && matches!(
                result.status,
                ProverStatus::Timeout | ProverStatus::InputError
            )
        {
            aggregate.status = result.status;
        }
    }
    aggregate.raw_output = format!(
        "% Subset audit: {} attempted of {} queued; {} pending; {} distinct contradictions. This does not establish whole-KB consistency.\n",
        results.len(), available, available.saturating_sub(results.len()), aggregate.contradiction_proofs.len()
    );
    for (index, result) in results.iter().enumerate() {
        aggregate.raw_output.push_str(&format!(
            "\n% Subset {}: {:?}\n{}",
            index + 1,
            result.status,
            result.raw_output
...
mod audit_tests {
    ...
    #[test]
    fn subset_satisfiability_is_not_whole_kb_consistency() {
        let result = ProverResult {
            status: ProverStatus::Consistent,
            ..Default::default()
        };
        assert_eq!(aggregate_audits(&[result], 1).status, ProverStatus::Unknown);
        assert_eq!(aggregate_audits(&[], 0).status, ProverStatus::Unknown);
    }

    #[test]
    fn deduplicates_support_and_keeps_source_ids() {
        let transcript = "# SZS status Unsatisfiable\n# SZS output start CNFRefutation\ncnf(c_0_1,axiom,p,file('input.p',kb_42)).\ncnf(c_0_2,axiom,~p,file('input.p',kb_43)).\ncnf(c_0_3,plain,$false,inference(sr,[],[c_0_1,c_0_2])).\n# SZS output end CNFRefutation\n";
        let result =
            result_from_transcript(transcript, "", ProverMode::CheckConsistency, Duration::ZERO);
        assert_eq!(result.status, ProverStatus::Inconsistent);
        assert!(result
            .proof_kif
            .iter()
            .any(|step| step.source_sid == Some(42)));
        let timeout = ProverResult {
            status: ProverStatus::Timeout,
            ..Default::default()
        };
        let aggregate = aggregate_audits(&[result.clone(), result, timeout], 5);
        assert_eq!(aggregate.status, ProverStatus::Inconsistent);
        assert_eq!(aggregate.contradiction_proofs.len(), 1);
        assert!(aggregate.raw_output.contains("2 pending"));
        let other = result_from_transcript(
            &transcript
                .replace("kb_42", "kb_44")
                .replace("kb_43", "kb_45"),
            "",
        ...
        let original =
            result_from_transcript(transcript, "", ProverMode::CheckConsistency, Duration::ZERO);
        assert_eq!(
            aggregate_audits(&[original, other], 2)
                .contradiction_proofs
                .len(),
            2
        );
