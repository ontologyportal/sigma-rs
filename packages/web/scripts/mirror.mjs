...
if (
  process.env.NO_REBUILD === "1" &&
  existsSync(join(SIGMAKEE_DIST, "sdk.mjs"))
) {
...
if (process.env.SKIP_VAMPIRE === "1") {
  ...
  vampireOk = false;
  console.warn(
    "==> Vampire WASM build failed -- the demo still runs, just without that backend.",
  );
}
...
const vampireComplete = VAMPIRE_FILES.every((f) =>
  existsSync(join(VAMPIRE_DIST, f)),
);
