...
export async function activate(context: ExtensionContext): Promise<void> {
  ...
  context.subscriptions.push(
    ...
    commands.registerCommand("sumo.showServerOutput", () =>
      outputChannel?.show(),
    ),
    ...
    commands.registerCommand("sumo.browseInSigma", () =>
      browseInSigmaCommand(() => state.get()),
    ),
    commands.registerCommand("sumo.formatAxiom", () =>
      formatAxiomCommand(() => client),
    ),
    commands.registerCommand("sumo.generateTPTP", () =>
      onGenerateTptp(context),
    ),
    commands.registerCommand("sumo.openRepl", () =>
      openReplCommand(ensureKernel(context)),
    ),
    ...
    commands.registerCommand("sumo.kernel.restart", () =>
      onRestartKernel(context),
    ),
    commands.registerCommand("sumo.kernel.showOutput", () =>
      outputChannel?.show(),
    ),
    commands.registerCommand("sumo.kernel.rebuildDatabase", () =>
      onRebuildKernelDb(context),
    ),
    commands.registerCommand("sumo.kernel.deleteDatabase", () =>
      onDeleteKernelDb(context),
    ),
    commands.registerCommand("sumo.kernel.reconcileOpenFiles", () =>
      onReconcileOpenFiles(context),
    ),
    commands.registerCommand("sumo.kernel.showFileStatus", () =>
      onShowKernelFileStatus(context),
    ),
    commands.registerCommand("sumo.debug.file", () =>
      onDebugActiveFile(context),
    ),
  );
    ...
    workspace.onDidChangeConfiguration(async (e: ConfigurationChangeEvent) => {
      if (
        ...
        outputChannel?.appendLine(
          "[extension] server config changed, restarting",
        );
        await stopClient();
...
function resolveServerBinary(
  ...
  if (userOverride && userOverride.length > 0) {
    outputChannel?.appendLine(
      `[extension] using user-configured server path: ${userOverride}`,
    );
  ...
  if (bundled && fs.existsSync(bundled)) {
    outputChannel?.appendLine(
      `[extension] using bundled server binary: ${bundled}`,
    );
    return bundled;
  }

  outputChannel?.appendLine(
    "[extension] no bundled server; falling back to `sumo-lsp` on PATH",
  );
  return "sumo-lsp";
}
...
function reloadConfig(): void {
  ...
  try {
    ...
    outputChannel?.appendLine(
      `[extension] failed to parse config.xml ${resolved}: ${err}`,
    );
    parsedConfig = null;
  }
}
...
function ensureKernel(context: ExtensionContext): SumoKernelClient {
  ...
  kernelClient = new SumoKernelClient(context, outputChannel!, () =>
 ...
 * unchanged content).
 */
function resolveKernelSpawn(context: ExtensionContext): {
  command: string;
  args: string[];
} {
  const config = workspace.getConfiguration("sumo");
...
async function refreshKernelHealth(): Promise<void> {
  ...
  if (health.status === "ok") {
    outputChannel?.appendLine(
      `[extension] kernel available at ${health.binaryPath}`,
    );
    return;
  }
...
function presentKernelHealthDialog(health: KernelHealth): void {
  ...
  switch (health.status) {
    ...
    case "crash": {
      ...
      outputChannel?.appendLine(
        `[extension] kernel start failed: ${health.detail}`,
      );
      void window
        .showErrorMessage(
 ...
 */
async function onShowKernelFileStatus(
  context: ExtensionContext,
): Promise<void> {
  const kernel = ensureKernel(context);
  try {
    ...
    outputChannel?.show(true);
    outputChannel?.appendLine(
      `[kernel] listFiles: ${files.length} file(s) loaded`,
    );
    for (const f of files) {
      outputChannel?.appendLine(
        `[kernel]   ${f.sentenceCount.toString().padStart(6)} sentences  ${f.path}`,
      );
    }
  } catch (err) {
...
async function disposeActiveIfTemp(reason: string): Promise<void> {
  ...
  if (!extensionContext || !outputChannel) {
    return;
  }
  outputChannel.appendLine(`[extension] disposing ephemeral KB (${reason})`);
  if (kernelClient) {
    try {
      await kernelClient.stop();
    } catch {
...
function onFileSaved(context: ExtensionContext, filePath: string): void {
  if (!kernelClient || !kernelClient.isRunning()) {
    return;
  }
  const active = state.get();
  const abs = normaliseAbs(filePath);
  if (!active || !active.files.has(abs)) {
    return;
  }
 ...
 */
function enclosingTopLevelForm(
  text: string,
  offset: number,
): string | undefined {
...
async function pushIgnoredDiagnosticsToServer(): Promise<void> {
  ...
  try {
    ...
    outputChannel?.appendLine(
      `[extension] setIgnoredDiagnostics failed: ${err}`,
    );
  }
}
...
async function classifyOpenedKif(doc: TextDocument): Promise<void> {
  ...
  if (file.endsWith(".kif.tq")) {
    return;
  }
  const norm = normaliseAbs(file);
  if (inflightPrompts.has(norm)) {
    return;
  }
...
async function applyPromptOutcome(outcome: PromptOutcome): Promise<void> {
  switch (outcome.kind) {
    ...
    case "load-config-here": {
      ...
      state.setConfigKb(kb.name, kb.files);
      outputChannel?.appendLine(
        `[extension] loaded "${kb.name}" (${kb.files.length} files) into this window`,
      );
      break;
    }
    ...
    case "new-temp-here": {
      ...
      outputChannel?.appendLine(
        `[extension] started temporary KB with ${outcome.file}`,
      );
      break;
    }

    case "add-to-active": {
      if (!state.addFile(outcome.file)) {
        return;
      }
      const active = state.get();
      outputChannel?.appendLine(
        `[extension] added ${outcome.file} to "${active?.name ?? "?"}"`,
      );
      if (active?.source === "config" && active.configKbName && parsedConfig) {
        await writeConfigKb(
          parsedConfig,
          active.configKbName,
          Array.from(active.files),
        );
      }
      break;
    }
...
async function openKbInNewWindow(kbName: string): Promise<void> {
  if (!parsedConfig) {
    window.showErrorMessage(
      "config.xml not loaded; cannot open KB in new window.",
    );
    return;
  }
  ...
  outputChannel?.appendLine(
    `[extension] opening "${kb.name}" in new window via ${wsPath}`,
  );
  await commands.executeCommand("vscode.openFolder", Uri.file(wsPath), {
    forceNewWindow: true,
  });
}
...
async function openTempInNewWindow(file: string): Promise<void> {
  ...
  outputChannel?.appendLine(
    `[extension] opening ${file} in new temp window via ${wsPath}`,
  );
  await commands.executeCommand("vscode.openFolder", Uri.file(wsPath), {
    forceNewWindow: true,
  });
...
interface KbPick extends QuickPickItem {
  kbName?: string;
  temp?: boolean;
  newWindow?: boolean;
}
...
async function onLoadKb(): Promise<void> {
  ...
  if (chosen.temp) {
    ...
    if (kb) {
      state.setConfigKb(kb.name, kb.files);
      outputChannel?.appendLine(
        `[extension] loaded "${kb.name}" (${kb.files.length} files)`,
      );
    }
  }
...
async function onAddFile(): Promise<void> {
  ...
  if (!active) {
    window.showInformationMessage(
      'No KB is active in this window.  Run "SUMO: Load KB…" first.',
    );
    return;
  }
...
async function onRemoveFile(...args: unknown[]): Promise<void> {
  ...
  outputChannel?.appendLine(
    `[extension] removed ${file} from "${active.name}"`,
  );
