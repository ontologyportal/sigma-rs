...
// lives here; every other module in the crate speaks sumo-kb types.
//
...
///
/// The tag string is what sumo-kb stores as `Sentence.file` and what
...
/// Convert a sumo-kb [`Span`] into an LSP [`Range`] relative to `rope`.
...
/// file tag.  Should never be observed in practice -- sumo-kb produces
