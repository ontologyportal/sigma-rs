...
use crate::semantics::{relation::RelationDomain, query::DocEntry};
...
impl KnowledgeBase {
    ...
    pub fn format_string(&self, relation: &str, language: Option<&str>) -> Vec<DocEntry> {
        ...
        self.layer.semantic.format(id, language)
    }
...
fn build_manpage(kb: &KnowledgeBase, sym_id: SymbolId, name: &str) -> ManPage {
    ...
    let parents = collect_parents(&kb.layer.semantic.syntactic, sym_id);
    let (arity, domains, range) = signature(kb, sym_id);
    let (ref_args, ref_nested) = collect_refs(&kb.layer.semantic.syntactic, sym_id);
...
fn signature(
    ...
    let range = kb.layer.semantic.range(sym_id).ok().flatten().map(|rd| sort_sig(kb, &rd));
...
fn sort_sig(kb: &KnowledgeBase, rd: &RelationDomain) -> SortSig {
    let id = rd.id();
    SortSig {
        class:    kb.layer.semantic.syntactic.sym_name(id).to_string(),
...
///
/// `SyntacticLayer::axiom_sentences_of` already recurses through
