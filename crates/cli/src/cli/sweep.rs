...
use sigmakee_rs_core::{
    parse_test_content, KnowledgeBase, NativeOpts, ProverLayer, ProverStatus, Severity, SineParams,
    Strategy,
};
...
pub fn run_sweep(
    ...
    std::thread::scope(|s| {
        for _ in 0..workers {
            s.spawn(|| loop {
                ...
                let t0 = Instant::now();
                let (result, solved) =
                    run_one(shared_kb, &problems[pi], strat, budget, max_steps, timeout);
    ...
    if !unsolved.is_empty() {
        println!(
            "Unsolved by every config (not a tuning problem): {}",
            unsolved.join(", ")
        );
    }
        ...
        Problem::Tptp {
            ...
            let ok = match expected.as_deref() {
                ...
                Some("CounterSatisfiable") | Some("Satisfiable") => {
                    r.status != ProverStatus::Proved && r.status != ProverStatus::InputError
                }
                Some("Unsatisfiable") | Some("ContradictoryAxioms") => {
                    matches!(r.status, ProverStatus::Proved | ProverStatus::Inconsistent)
                }
