// crates/core/src/persist/load.rs
//
// Reconstruct a `SyntacticLayer` from the LMDB-backed persistent store.
//
...
//     recursively into `SyntacticLayer` using `alloc_sentence`.

use std::collections::HashMap;

use crate::{KbError, Span};
...
/// Load all formulas from `env` into a fresh `SyntacticLayer`.
///
/// Returns:
/// - The reconstructed `SyntacticLayer` (with `seed_counters` already called).
...
pub(crate) fn load_from_db(
    env: &LmdbEnv,
) -> Result<(SyntacticLayer, HashMap<SentenceId, Option<String>>), KbError> {
    let txn = env.read_txn()?;

    // Load symbols
    let stored_syms = env.all_symbols(&txn)?;
    crate::log!(Info, "sigmakee_rs_core::persist", format!("load_from_db: loading {} symbols", stored_syms.len()));
    ...
    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Info, target: "sigmakee_rs_core::persist", message: format!("load_from_db: loading {} formulas", stored_formulas.len()) });
    ...
    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Info, target: "sigmakee_rs_core::persist", message: format!("load_from_db: {} sentences, {} symbols; counters seeded at sym={} sent={}", store.sentences.len(), store.symbols.len(), max_sym_id + 1, max_sent_id + 1) });

    Ok((store, session_map))
}

// -- Allocate a StoredFormula into the SyntacticLayer --------------------------------
