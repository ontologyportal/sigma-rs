...
impl<'src> Tokenizer<'src> {
    ...
    fn classify_word(w: String) -> TokenKind {
        match w.as_str() {
            ...
            _ => {
                // Number: optional leading minus, then digits, optional dot+digits
                if is_numeric(&w) {
                    TokenKind::Number(w)
                } else {
                    TokenKind::Symbol(w)
...
/// Tokenize `src` and return all tokens (including synthetic EOF) plus any
/// hard errors encountered.  Tokenization continues after an error so that
/// callers can collect as many issues as possible.
...
mod tests {
    ...
    fn comment_skipped() {
        ...
        assert_eq!(kinds.len(), 3); // LParen, Symbol("foo"), RParen
    }
}
