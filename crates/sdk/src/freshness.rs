...
pub fn snapshot_git_tracked<L: TopLayer>(
    session: &Session<L>,
    ...
    for file in kb.iter_files() {
        if let Some(FileOrigin::Git(prov)) = kb.file_origin(&file) {
            out.entry((prov.uri, prov.branch))
                .or_default()
                .push((file, prov.commit));
        }
    }
    out
}
...
#[cfg(feature = "git")]
pub fn check_git_tracked(
    snapshot: &HashMap<(String, String), Vec<(String, String)>>,
) -> Vec<FreshnessReport> {
    let mut out = Vec::new();
...
fn check_local<L: TopLayer>(
    kb: &sigmakee_rs_core::KnowledgeBase<L>,
    file: &str,
) -> FreshnessReport {
    ...
    match std::fs::read(path) {
        ...
        Ok(_) => FreshnessReport {
            label,
            freshness: Freshness::Modified,
        },
        Err(_) => FreshnessReport {
            label,
            freshness: Freshness::Missing,
        },
    }
...
mod tests {
    ...
    fn tmp_kif(name: &str, contents: &str) -> std::path::PathBuf {
        let p =
            std::env::temp_dir().join(format!("sdk-freshness-{name}-{}.kif", std::process::id()));
    ...
    fn unchanged_file_reports_unchanged() {
        ...
        let r = reports
            .iter()
            .find(|r| r.label == p.to_string_lossy())
            .expect("file tracked");
