...
fn scope_of_buffer_added_sentences() {
    ...
    kb.stage(
        SourceFile::kif(PathBuf::from("base.kif"), base.into()),
        "base.kif",
    );
    kb.commit("base.kif");
    ...
    kb.stage(
        SourceFile::kif(PathBuf::from("base.kif"), text.into()),
        "base.kif",
    );
    kb.commit("base.kif");
    ...
    assert!(
        !session_scope
            .iter()
            .any(|d| d.code == "no-entity-ancestor" && d.message.contains("NewThing")),
