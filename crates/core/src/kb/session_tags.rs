...
/// integrated-prover path. Distinct from `__query__` so concurrent
/// subprocess + embedded asks on the same KB don't collide.
pub const SESSION_QUERY_EMBEDDED:   &str = "__query_embedded__";

/// Session used by `reload_kif` when feeding newly-added ASTs through
/// the normal ingest pipeline. Promoted to axiom status via
/// `make_session_axiomatic` before the reconcile returns.
pub const SESSION_RECONCILE_ADD:    &str = "__reconcile_add__";

/// Session used by the native CLI's `open_or_build_kb_profiled` to
/// tag sentences from `-f` / `-d` files before promoting them in bulk
/// via `make_session_axiomatic` at the end of the load pass.
pub const SESSION_FILES:            &str = "__files__";

/// Session used by `build_kb_from_files` for in-memory assembly from a
/// `-f` / `-d` set when the LMDB path is bypassed (no-db mode). The
/// trailing promote converts these to axioms.
pub const SESSION_BASE:             &str = "__base__";

/// Session used by `sumo load` (both flush + reconcile paths) to stage
/// freshly-parsed sentences before committing them to LMDB.
