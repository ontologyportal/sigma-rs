...
use sumo_sdk::{AskOp, IngestOp, ProverBackend, ProverStatus, SdkError};
...
fn missing_vampire_returns_vampire_not_found() {
    ...
    let result = AskOp::new(&mut kb, "(instance Fido Animal)")
        .vampire_path("/tmp/sumo-sdk-no-such-vampire-xxxxx")
        .timeout_secs(5)
        .run();
...
fn progress_callback_fires_around_ask() {
    ...
    impl sumo_sdk::ProgressSink for Sink {
        fn emit(&self, e: &sumo_sdk::ProgressEvent) {
            let label = match *e {
                sumo_sdk::ProgressEvent::AskStarted  { .. } => "ask-started",
                sumo_sdk::ProgressEvent::AskFinished { .. } => "ask-finished",
                _ => "other",
            };
