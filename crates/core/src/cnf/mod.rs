...
//   These are detected by name prefix and interned into the `SyntacticLayer`
//   with `is_skolem = true`.
...
use skolem::*;
use clause::{Clause, CnfLiteral, CnfTerm};
...
/// like `s__holds__1`) that the `SyntacticLayer` has not yet seen.  Those are
...
pub(crate) fn sentence_to_clauses(
    ...
    Ok(translate_ir_clauses(&mut layer.semantic.syntactic, &ir_clauses))
}
...
pub(crate) fn clausify_ir(
    ...
    let mut conv = NativeConverter::new(layer, Mode::Tff);
...
#[derive(Default)]
struct TranslateCtx {
    /// Cache: Vampire functor name -> sigmakee-rs-core SymbolId.  Keeps translation
...
mod tests {
    ...
    /// Parse `kif`, build a TranslationLayer wrapping the chain, and
    /// return it plus the id of the last root sentence.
    fn load_one(kif: &str) -> (TranslationLayer, SentenceId) {
        let mut store = SyntacticLayer::default();
        let errs = load_kif(&mut store, kif, "test");
        assert!(errs.is_empty(), "parse errors: {:?}", errs);
        let semantic = SemanticLayer::new(store);
        let sid = *semantic.syntactic.roots.last().expect("no sentence parsed");
        let layer = TranslationLayer::new(semantic);
        (layer, sid)
    }
