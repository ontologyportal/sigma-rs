...
export async function createKbCommand(
  ...
  const seed = await window.showInputBox({
    ...
    prompt:
      "Optional seed .kif file (absolute path or path relative to kbDir).  Leave blank for an empty KB.",
    placeHolder: "",
    ignoreFocusOut: true,
    validateInput(value) {
      const trimmed = value.trim();
  ...
  if (seedTrim.length > 0) {
    const abs = path.isAbsolute(seedTrim)
      ? seedTrim
      : path.join(cfg.kbDir, seedTrim);
