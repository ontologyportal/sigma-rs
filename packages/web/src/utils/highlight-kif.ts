 ...
 *  tokenized by Monaco instead (see services/monaco.ts's KIF_MONARCH, which
 *  mirrors this). */
 ...
 *  them would only offer dead ends. Shared with `highlight-tptp.ts`. */
export function isInternalSymbol(word: string): boolean {
...
/** Tokenize `src` into `tok-*` spans; `focusSymbol` is wrapped in `.sym-focus`
 *  instead of linked, and `linkSymbols` wraps every other symbol in an
 *  `a.sym-link[data-sym]` for delegated man-page navigation. */
export function highlightKif(
  src: string,
  {
    focusSymbol,
    linkSymbols,
