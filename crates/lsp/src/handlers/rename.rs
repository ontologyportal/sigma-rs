...
use crate::conv::{
    offset_to_position, position_to_offset, span_to_range_with_fallback, tag_to_uri, uri_to_tag,
};
...
fn rename_variable_in_document(
    ...
    let replacement = if is_row {
        format!("@{}", stripped)
    } else {
        format!("?{}", stripped)
    };
    let old_token = if is_row {
        format!("@{}", name)
    } else {
        format!("?{}", name)
    };
