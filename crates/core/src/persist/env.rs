...
//   - No `next_seq("sym")` / `next_seq("formula")`: stable IDs from SyntacticLayer counters
...
pub(crate) struct StoredFormula {
    /// Stable `SentenceId` assigned by `SyntacticLayer`.
    pub id:        SentenceId,
    ...
    /// File/session tag used to group sentences in `SyntacticLayer`.
    pub file:      String,
}
...
use crate::trans::ArithCond;
#[cfg(feature = "ask")]
use crate::trans::SortAnnotations;
...
pub(crate) struct CachedTaxonomy {
    pub kb_version:           u64,
    pub tax_edges:            Vec<crate::semantics::taxonomy::TaxEdge>,
    pub numeric_sort_cache:   std::collections::HashMap<SymbolId, crate::trans::Sort>,
...
impl LmdbEnv {
    pub(crate) fn open(path: &Path) -> Result<Self, KbError> {
        crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Info, target: "sigmakee_rs_core::persist", message: format!("opening LMDB at {}", path.display()) });
        ...
        // legacy layout.
        {
            ...
            if has_formulas && schema_marker.is_none() {
                return Err(KbError::SchemaMigrationRequired(format!(
                    "LMDB at {} was created by an older build of sigmakee-rs-core \
        ...
        wtxn.commit()?;
        crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core::persist", message: format!("LMDB opened; schema v{}, max_dbs={}", SCHEMA_VERSION, MAX_DBS) });
        ...
        let added_features: Vec<&'static str> = {
            ...
            match manifest {
                None => {
                    ...
                    // `write_axioms` call.
                    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core::persist", message: format!("no feature manifest present; will stamp on next commit (features={:?})", current) });
                    Vec::new()
                }
                Some(m) if m.features == current => {
                    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core::persist", message: format!("feature manifest matches current build: {:?}", current) });
                    Vec::new()
                }
                Some(m) => {
                    ...
                    if !removed.is_empty() {
                        crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Warn, target: "sigmakee_rs_core::persist", message: format!("feature drift: DB was written with {:?} but this build \
                             lacks those features.  The corresponding LMDB tables \
                             will sit unused; data is preserved across reopens.", removed) });
                    }
                    if !added.is_empty() {
                        crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Info, target: "sigmakee_rs_core::persist", message: format!("feature drift: DB was written WITHOUT {:?} but this build \
    ...
    pub(crate) fn put_symbol(
        ...
        if self.get_symbol_id(rtxn, &sym.name)?.is_some() {
            ...
            crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Trace, target: "sigmakee_rs_core::persist", message: format!("put_symbol: '{}' already in DB", sym.name) });
            return Ok(());
        }
        #[cfg(debug_assertions)]
        crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Trace, target: "sigmakee_rs_core::persist", message: format!("put_symbol: '{}' id={}", sym.name, sym.id) });
    ...
    pub(crate) fn put_formula(
        ...
        #[cfg(debug_assertions)]
        crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Trace, target: "sigmakee_rs_core::persist", message: format!("put_formula: id={}", formula.id) });
    ...
    pub(crate) fn delete_formula(
        ...
        #[cfg(debug_assertions)]
        crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Trace, target: "sigmakee_rs_core::persist", message: format!("delete_formula: sid={} removed", sid) });
        Ok(())
    }
    ...
    pub(crate) fn intern_clause(
        ...
        if let Some(bytes) = self.clause_hashes.get(rtxn, &hash_key)? {
            ...
            #[cfg(debug_assertions)]
            crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Trace, target: "sigmakee_rs_core::persist", message: format!("intern_clause: hash={:016x} -> existing id={}", canonical, id) });
            return Ok(id);
        }
        ...
        #[cfg(debug_assertions)]
        crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Trace, target: "sigmakee_rs_core::persist", message: format!("intern_clause: hash={:016x} -> new id={}", canonical, id) });
        Ok(id)
    }
    ...
    pub(crate) fn get_cache<T: for<'de> Deserialize<'de>>(
        ...
        match bincode::deserialize::<T>(bytes) {
            Ok(v)  => Ok(Some(v)),
            Err(e) => {
                crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Warn, target: "sigmakee_rs_core::persist", message: format!("cache '{}' deserialize failed ({}); treating as absent", key, e) });
                Ok(None)
            }
        }
    }
    ...
    pub(crate) fn all_formulas(&self, txn: &RoTxn) -> Result<Vec<StoredFormula>, KbError> {
        ...
        crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core::persist", message: format!("all_formulas: {} formula(s) loaded", out.len()) });
        Ok(out)
    }
...
mod tests {
    ...
    fn tmp_dir(name: &str) -> std::path::PathBuf {
        ...
        p.push(format!("sigmakee-rs-core-phase4-{}-{}-{}",
    ...
    /// binary was compiled with.  The sigmakee-rs-core test suite runs with
    ...
    fn manifest_roundtrips_across_reopen() {
        use crate::syntactic::{load_kif, SyntacticLayer};
