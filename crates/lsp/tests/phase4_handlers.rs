...
use lsp_types::{
    ...
    request::{Initialize, References, Rename, Shutdown, WorkspaceSymbolRequest},
    DidOpenTextDocumentParams, InitializeParams, InitializedParams, Location, PartialResultParams,
    Position, ReferenceContext, ReferenceParams, RenameParams, SymbolKind, TextDocumentIdentifier,
...
fn references_returns_every_occurrence_of_symbol() {
    ...
    let refs = references_at(&client, &uri, 0, 11, true, 10).expect("references found");
...
fn references_excludes_declaration_when_asked() {
    ...
    let with_decl = references_at(&client, &uri, 0, 11, true, 11).expect("with decl");
    let no_decl = references_at(&client, &uri, 0, 11, false, 12).expect("no decl");
    assert!(
...
fn rename_symbol_replaces_every_occurrence() {
    ...
    let edit = rename_at(&client, &uri, 0, 11, "Person", 20).expect("rename produced an edit");
...
fn rename_variable_preserves_sigil() {
    ...
    let edit = rename_at(&client, &uri, 0, 9, "Y", 30).expect("variable rename");
    ...
    for e in edits {
        assert!(
            ...
            "expected sigil-preserved rename, got '{}'",
            e.new_text
        );
...
fn rename_variable_respects_scope() {
    ...
    let edit = rename_at(&client, &uri, 0, 9, "Y", 40).expect("scoped rename");
    let changes = edit.changes.expect("changes");
    let edits = changes.get(&uri).expect("this URI");
    ...
    for e in edits {
        assert_eq!(
            ...
            "scope leak: rename touched line {}",
            e.range.start.line
        );
    }
...
fn workspace_symbols_filters_by_substring() {
    ...
    assert!(
        !names.contains(&"Mammal"),
        "unexpected Mammal in {:?}",
        names
    );
