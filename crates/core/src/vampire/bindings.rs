// crates/core/src/vampire/bindings.rs
//
...
pub fn extract_bindings(proof: &Proof, qvm: &QueryVarMap) -> Vec<ProofBinding> {
    ...
    let target_vars: Vec<String> = qvm
        ...
        .map(|&i| format!("X{}", i))
        .collect();
    crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core::bindings", message: format!("extract_bindings: {} steps, target_vars={:?}, idx_to_kif={:?}", steps.len(), target_vars, qvm.idx_to_kif) });
    for (i, (f, rule, prems)) in steps.iter().enumerate() {
        crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Trace, target: "sigmakee_rs_core::bindings", message: format!("  step {}: rule={:?} prems={:?} conclusion={:?}", i, rule, prems, f) });
    }

    // Identify the conjecture-input step.
    let Some(neg_conj_idx) = find_negated_conjecture(&steps) else {
        crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core::bindings", message: format!("no conjecture-like step in proof; cannot extract bindings") });
        return Vec::new();
    };
...
impl AliasTracker {
    ...
    fn finalise(&self, qvm: &QueryVarMap) -> Vec<ProofBinding> {
        ...
        crate::emit_event!(crate::progress::ProgressEvent::Log { level: crate::progress::LogLevel::Debug, target: "sigmakee_rs_core::bindings", message: format!("finalise: extracted {} binding(s)", out.len()) });
        out
    }
}
