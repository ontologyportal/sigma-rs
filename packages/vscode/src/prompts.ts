...
async function promptNoActiveKb(
  ...
  const header =
    memberOfConfigKbs.length > 0
      ? `This file belongs to ${
          memberOfConfigKbs.length > 1
            ? `${memberOfConfigKbs.length} declared KBs`
            : `KB "${memberOfConfigKbs[0]}"`
...
async function promptActiveKbMismatch(
  ...
  for (const kbName of memberOfConfigKbs) {
    if (active.source === "config" && active.configKbName === kbName) {
      continue;
    }
    picks.push({
    ...
    memberOfConfigKbs.length > 0
      ? `This file belongs to ${
          memberOfConfigKbs.length > 1
            ? `${memberOfConfigKbs.length} declared KBs`
            : `KB "${memberOfConfigKbs[0]}"`
        } — but this window is editing "${active.name}".  Choose how to handle it.`
