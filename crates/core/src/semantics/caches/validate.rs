                ...
                Event::ValidateKB => {
                    ...
                    for sid in roots {
                        store.update(
                            ...
                            Arc::new(v.validate_sentence_collect(sid)),
                        );
                    }
                }
...
#[cfg(test)]
mod tests {
    use crate::cache::{events::Event, CacheConfig};
    ...
    fn validation_memoises_per_sentence() {
        ...
        assert!(
            !errs.is_empty(),
            "undeclared-relation head should raise a diagnostic"
        );
        // Cached after first access (under the Base scope).
        assert!(
            layer.validate.peek(&base(sid)).is_some(),
