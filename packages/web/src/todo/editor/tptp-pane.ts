 ...
 * `refreshTptpPane`/`scheduleTptpRefresh`/`scheduleTptpFollow` stay the entry
 * points `tabs/edit.ts` calls into.
 */
...
export const tptpPaneOpen = ref(false);
export const tptpStatus = ref('');
export const tptpMenuOpen = ref(false);
export const tptpMenuPos = ref({ left: '0px', top: '0px' });
...
async function setTptpPaneOpen(on) {
  if (on === tptpPaneOpen.value) return;
  tptpPaneOpen.value = on;
...
function setTptpMenuOpen(on) {
  if (on) {
    ...
    tptpMenuPos.value = { left: `${Math.round(r.left)}px`, top: `${Math.round(r.bottom + 4)}px` };
  }
  tptpMenuOpen.value = on;
}

// While a split view is open, the button acts as a plain dismiss — the menu
// only appears when there is something to choose.
export function onEditTptpToggleClick() {
  if (tptpPaneOpen.value) { setTptpPaneOpen(false); return; }
  setTptpMenuOpen(!tptpMenuOpen.value);
}
...
export function onTptpTranslationBtnClick() {
  setTptpMenuOpen(false);
  setTptpPaneOpen(!tptpPaneOpen.value);
}
...
export async function refreshTptpPane() {
  if (!tptpPaneOpen.value || !tptpEditor) return;
  tptpStatus.value = 'generating…';
  try {
    ...
    tptpStatus.value = `${tptpEditor.getModel().getLineCount()} lines`;
    followTptpCursor();
  } catch (e) {
    tptpStatus.value = 'translation failed: ' + (e && e.message || e);
  }
}
...
async function followTptpCursor() {
  ...
  if (!tptpPaneOpen.value || !tptpEditor || !editor || !tptpPaneUsable()) return;
