...
suite("sumo-lsp integration", () => {
  ...
  test("hover renders the man-page markdown", async () => {
    ...
    assert.ok(
      md.includes("member of the species"),
      `documentation present, got: ${md}`,
    );
  ...
  test("goto-definition resolves across constituents", async () => {
    ...
    const factsDoc = await vscode.workspace.openTextDocument(
      fixtureFile("facts.kif"),
    );
    const locations = await waitFor("definition of Human from facts.kif", () =>
      vscode.commands.executeCommand<vscode.Location[]>(
        "vscode.executeDefinitionProvider",
        factsDoc.uri,
        posOf(factsDoc, "Human"),
      ),
    );
  ...
  test("references span every constituent", async () => {
    ...
    assert.ok(
      files.has("base.kif") && files.has("facts.kif"),
  ...
  test("symbol rename edits every occurrence in every file", async () => {
    ...
    for (const [, edits] of entries) {
      for (const e of edits) {
        assert.strictEqual(e.newText, "Person");
      }
    }
  });
  ...
  test("variable rename stays inside its form and keeps the sigil", async () => {
    ...
    assert.strictEqual(
      edits.length,
      2,
      `both ?X occurrences, got ${edits.length}`,
    );
    for (const e of edits) {
      assert.strictEqual(e.newText, "?Y");
    }
  });
  ...
  test("semantic diagnostics are published for KB files", async () => {
    ...
    await editor.edit((b) =>
      b.insert(
        new vscode.Position(baseDoc.lineCount, 0),
        "\n(subclass Widget)\n",
      ),
    );
    try {
  ...
  test("formatting pretty-prints the document", async () => {
    ...
    try {
      ...
      assert.ok(
        text.startsWith("(subclass Human Hominid)"),
        `content preserved, got: ${text.slice(0, 80)}`,
      );
      assert.ok(
        text.includes("(=> (instance ?X Human) (instance ?X Animal))"),
