...
//! [`sumo_kb::TestCase`], runs the conjecture against the configured
...
//!
//! Each case gets its own session (`"sumo-sdk-test-{idx}"`) which is
...
use sumo_kb::{parse_test_content, KnowledgeBase, TestCase, TptpLang, VampireRunner};
...
impl<'a> TestOp<'a> {
    ...
    /// Add a pre-parsed [`sumo_kb::TestCase`].  Useful when the
...
fn run_one(
    ...
    let timings_zero = sumo_kb::prover::ProverTimings::default();
    ...
    let session  = format!("sumo-sdk-test-{}", idx);
    let load_tag = format!("sumo-sdk-test-src-{}", idx);
    ...
    let proved = matches!(result.status, sumo_kb::ProverStatus::Proved);
    ...
    if matches!(result.status, sumo_kb::ProverStatus::Unknown) && !proved && !result.raw_output.is_empty() {
