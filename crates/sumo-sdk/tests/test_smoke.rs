...
use sumo_sdk::{IngestOp, TestOp, TestOutcome};
...
fn missing_test_file_aborts_run_with_io_error() {
    ...
    let result = TestOp::new(&mut kb)
        .add_file("/tmp/sumo-sdk-no-such-test-xxxxx.kif.tq")
        .run();
    assert!(matches!(result, Err(sumo_sdk::SdkError::Io { .. })));
}
...
fn missing_test_dir_aborts_run_with_dir_read_error() {
    let mut kb = build_kb();
    let result = TestOp::new(&mut kb)
        .add_dir("/tmp/sumo-sdk-no-such-test-dir-xxxxx")
        .run();
    assert!(matches!(result, Err(sumo_sdk::SdkError::DirRead { .. })));
}
...
fn dir_walk_finds_only_kif_tq_files() {
    ...
    let report = match vampire_on_path() {
        ...
        None     => report
            ...
            .vampire_path("/tmp/sumo-sdk-definitely-not-vampire")
            .run()
            .unwrap(),
    };
