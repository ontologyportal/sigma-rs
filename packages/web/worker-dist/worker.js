...
// _utils/session.ts
var SESSION_MAX_AGE_S = 30 * 24 * 60 * 60;
var SESSION_COOKIE = "session_id";
function getCookie(request, name) {
  const cookie = request.headers.get("Cookie") || "";
  const match2 = cookie.match(new RegExp(`(?:^|; )${name}=([^;]*)`));
...
async function createSession(accessToken, env) {
  const sessionId = crypto.randomUUID();
  const data = { access_token: accessToken, created_at: Date.now() };
...
function deleteSession(sessionId, env) {
...
async function getGithubToken(request, env) {
    ...
    await fetch(
      `https://api.github.com/applications/${context.env.GITHUB_OAUTH_CLIENT_ID}/token`,
      {
        ...
        headers: {
          ...
          Authorization: "Basic " + btoa(`${context.env.GITHUB_OAUTH_CLIENT_ID}:${context.env.GITHUB_OAUTH_CLIENT_SECRET}`),
          "Content-Type": "application/json"
        },
...
// ../../../.wrangler/tmp/pages-TN1Its/functionsRoutes-0.2484740755705439.mjs
var routes = [
  {
  ...
  {
    routePath: "/api/github-auth-callback",
    mountPath: "/api",
    method: "GET",
    middlewares: [],
